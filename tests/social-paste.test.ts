...
import { connectSocial, disconnectSocial } from "../src/lib/social-accounts";
import {
  parseBlueskyHandle,
  parseDiscordWebhook,
...
test("paste fields reject tracking URLs and private hosts", () => {
  ...
  expect(
    parseDiscordWebhook(
      "https://discord.com/api/webhooks/123456789012345678/abcdefghijklmnopqrstuvwxyz12",
    ),
...
test("paste secrets stay encrypted and require an explicit connection", async () => {
  const scope = "test-paste-scope";
  localStorage.clear();
  await savePasteSecret(
    scope,
    parsePasteSecret("bluesky", {
  ...
  await connectSocial(scope, "bluesky");
...
test("paste post talks to Bluesky session then createRecord, never retries", async () => {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes("createSession"))
      return new Response(JSON.stringify({ accessJwt: "jwt", did: "did:plc:test" }), {
        status: 200,
      });
