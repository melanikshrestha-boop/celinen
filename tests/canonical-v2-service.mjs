...
assert.equal((await post(jpeg, {'Content-Type':'application/octet-stream'})).status,401);
if (args.includes('--dark')) {
  assert.equal(ready.enabled,false); const r = await post(jpeg); assert.equal(r.status,503);
  assert.equal((await r.json()).code,'disabled'); console.log('PASS dark /health /ready, unauthenticated and disabled processing');
} else {
  ...
  assert.equal((await post(jpeg,{...headers(jpeg),'Content-Type':'image/jpeg'})).status,400);
  assert.equal((await post(jpeg,{...headers(jpeg),'X-Job-Id':'../invalid'})).status,400);
