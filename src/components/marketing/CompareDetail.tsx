...
export function CompareDetail({ item }: { item: CompareEntry }) {
  ...
  return (
    <article className="celinen-compare-detail">
      <p className="celinen-compare__eyebrow">Comparison</p>
      <div className="celinen-compare-detail__vs">
        <LogoMark size={36} />
        vs
      ...
      <p className="celinen-compare-detail__lede">{item.blurb}</p>
      <div className="celinen-compare-detail__actions">
        <Link to="/auth" search={{ next: "/dashboard", mode: "signin" }}>
          Try {PRODUCT_NAME} free
        </Link>
        <Link to="/compare">All comparisons</Link>
      </div>
      <div className="celinen-compare-detail__score">
      ...
      <p className="celinen-compare-detail__lede">{item.lede}</p>
      <h2>Where each one is stronger.</h2>
      <p className="celinen-compare-detail__lede">
      ...
      <table className="celinen-compare-detail__table">
        <thead>
          <tr>
            <th>{PRODUCT_NAME}</th>
            <th>{item.name}</th>
      ...
      <p className="celinen-compare-detail__sources">{item.sources}</p>
      <div className="celinen-compare-detail__choose">
        <div>
      ...
      <section className="celinen-compare__more">
        ...
        <div className="celinen-compare__grid">
          {others.map((entry) => (
            <Link
              ...
              className="celinen-compare__card"
            >
      ...
      <section className="celinen-compare__try">
        <h2>Switching from {item.name}?</h2>
        <p>Import the card. Pick. Send tonight.</p>
        <div className="celinen-compare__actions">
          <Link to="/auth" search={{ next: "/dashboard", mode: "signin" }}>
            Get started
          </Link>
