...
import { clientCsv, downloadText, dueClients, localDate, nextFollowUp, reminderCalendar } from "@/lib/business/reminders";
...
function Clients() {
  ...
  useEffect(() => setLocal(new URLSearchParams(window.location.search).get("legacy") === "1"), []);
  if (local === null)
    return (
      <Shell>
...
export function LocalClients({ cloud = false }: { cloud?: boolean }) {
  ...
  useToolLeaveGuard(busy ? "A client or booking save is still in progress." : editing || bookingEdit ? "You have an open client or booking form. Any unsaved changes will be discarded." : null);

  useEffect(() => {
    let alive = true;
    const readClients = async () => {
      const loaded = cloud
        ? await readBusinessClients().then(state => ({ ok: true as const, state })).catch(e => ({ ok: false as const, error: e instanceof Error ? e.message : "Client sync is unavailable." }))
        : loadClientWorkspace();
      if (!alive) return;
      if (loaded.ok) {
  ...
  async function saveClient(next: WorkspaceClient, revision = state.revision): Promise<boolean> {
    ...
    try {
      ...
      const result = cloud
        ? await saveBusinessClients({ data: update }).then(state => ({ ok: true as const, state })).catch(e => ({ ok: false as const, error: e instanceof Error ? e.message : "Client save failed." }))
  ...
  return (
    <Shell>
      <SectionTitle
        ...
        sub={cloud ? "Your clients, their shoots, and the next conversation." : "Previous device-only records. These are not synced or assigned to your signed-in account."}
      />
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <a className="text-sm text-moss underline underline-offset-4" href={cloud ? "/clients?legacy=1" : "/clients"}>{cloud ? "Previous device records" : "Account clients"}</a>
        ...
        <button type="button" className="px-2 py-2 text-sm text-moss hover:text-ink" aria-pressed={onlyDue} onClick={() => setOnlyDue(!onlyDue)}>
          {onlyDue ? "Show everyone" : `${dueClients(state.clients, today).length} follow-ups due`}
        </button>
        <details className="relative ml-auto text-sm">
          ...
          <div className="absolute right-0 z-10 mt-2 w-64 rounded-xl bg-card p-4 shadow-lg">
            <button disabled={!ready} className="block py-2" onClick={() => downloadText(clientCsv(state.clients), "lenslabs-clients.csv", "text/csv;charset=utf-8")}>Export spreadsheet</button>
            <button disabled={!ready || !state.clients.some(c => c.followUpOn)} className="block py-2" onClick={() => downloadText(reminderCalendar(state.clients), "lenslabs-follow-ups.ics", "text/calendar;charset=utf-8")}>Add follow-ups to calendar</button>
            <p className="mt-2 text-sm text-moss">Import into your calendar for alerts while LensLabs is closed. Re-export after changing dates; this is not a live sync.</p>
          </div>
        </details>
        {notice && (
      ...
      <div className={`grid gap-8 ${client || editing ? "xl:grid-cols-[minmax(380px,1fr)_minmax(320px,1fr)]" : ""}`}>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm"><thead className="text-moss"><tr>
            <th className="px-3 py-3 font-normal">Client</th><th className="px-3 py-3 font-normal">Stage</th><th className="px-3 py-3 font-normal">Next follow-up</th>
          </tr></thead><tbody>
          {visibleClients.map((entry) => (
            <tr key={entry.id} className={selected === entry.id ? "bg-muted/50" : "hover:bg-muted/30"}>
            ...
            >
              <p className="font-display text-[15px] font-semibold tracking-tight">{entry.name}</p>
              <p className="text-[13px] text-moss">
                {entry.org || entry.email || "Contact not added"}
              </p>
            </button></td>
            <td className="px-3 py-3"><select aria-label={`Stage for ${entry.name}`} className="max-w-28 bg-transparent py-2 capitalize" disabled={busy || !!editing || !!bookingEdit} value={entry.stage} onChange={e => void saveClient({ ...entry, stage: e.target.value as ClientStage, updatedAt: new Date().toISOString() })}>{CLIENT_STAGES.map(stage => <option key={stage}>{stage}</option>)}</select></td>
            <td className="px-3 py-3"><input aria-label={`Follow-up for ${entry.name}`} type="date" className={`w-36 bg-transparent py-2 ${entry.followUpOn && entry.followUpOn <= today ? "text-rust" : "text-moss"}`} disabled={busy || !!editing || !!bookingEdit} value={entry.followUpOn ?? ""} onChange={e => void saveClient({ ...entry, followUpOn: e.target.value || null, updatedAt: new Date().toISOString() })} />
            {dueClients([entry], today).length > 0 && <div className="flex gap-3 text-sm"><button disabled={busy || !!editing || !!bookingEdit} onClick={() => void saveClient({ ...entry, followUpOn: null })}>Done</button><button disabled={busy || !!editing || !!bookingEdit} className="text-moss" onClick={() => void saveClient({ ...entry, followUpOn: nextFollowUp(today) })}>Next week</button></div>}</td>
            </tr>
          ))}
          </tbody></table>
          ...
          {editing ? (
            <Card>
              <h2 className="font-display text-lg font-semibold">
                {editing === "new" ? "Add a lead" : "Edit client"}
              </h2>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <label className={label}>
                    ...
                    <select
                      className={field}
                      value={draft.stage}
                      onChange={(e) => setDraft({ ...draft, stage: e.target.value as ClientStage })}
                    >
                      {CLIENT_STAGES.map((stage) => (
            ...
            <>
              ...
              {preview && (
                <p role="status" className="text-[13px] text-moss">
                  Client portal preview — only on this device. No public link, invitation, payment,
                  or client access has been created.
                </p>
              )}
              <Card className="p-0">
                    ...
                    preview ? (
                      ...
                      <label key={gallery.id} className="flex items-center gap-3 text-sm">
                        <input
                          ...
                          onChange={(event) =>
                            void saveClient(
                              linkClientDraft(client, "gallery", gallery.id, event.target.checked),
                            )
                          }
                        />
                        <span>
              ...
              <Card>
                    ...
                    <label key={invoice.id} className="flex items-start gap-3 text-sm">
                      {!preview && (
                        <input
                          ...
                          onChange={(event) =>
                            void saveClient(
                              linkClientDraft(client, "invoice", invoice.id, event.target.checked),
                            )
                          }
                        />
                      )}
                      ...
                      <span className="font-mono text-[13px]">{dollars(invoice.amountCents)}</span>
                    </label>
                  ))}
                </div>
                ...
                {client.invoiceIds.some((id) => !invoices.some((invoice) => invoice.id === id)) && (
                  <p className="mt-2 text-[12px] text-moss">
            ...
            ready && (
              <Card>
                <p className="text-sm text-moss">
                  Add a lead, record the brief, then turn it into a booking. Your clients will
                  appear here.
                </p>
              </Card>
            )
