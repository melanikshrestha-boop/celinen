...
export async function createPublication(owner: string, input: PublicationInput) {
  ...
  validatePublicSelection(room.state, input.versionIds, input.instagram);
  const account = input.instagram ? await instagramConnection(owner) : null;
  if (input.instagram && (!account || Date.parse(account.expires_at) < Date.now() + 60000))
...
export async function runPublication(
  ...
  try {
    ...
    const versions = validatePublicSelection(room.state, post.versionIds, post.instagram);
    ...
    if (post.instagram && post.destinations.instagram.status !== "published") {
      try {
        ...
        post.destinations.instagram = { status: "preparing", note: "Preparing Instagram images…" };
        await save();
        if (!post.containerId) {
          const ids: string[] = post.childContainers ?? [];
          for (const path of post.paths.slice(ids.length)) {
...
export async function publicPortfolio(id: string, db = businessDatabase()) {
  ...
  if (error || !post || post.destinations.portfolio.status !== "published")
