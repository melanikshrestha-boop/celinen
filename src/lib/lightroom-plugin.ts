/**
 * Celinen Lightroom Classic plugin (Lua source, generated in the browser).
 *
 ...
 *     payload to the Celinen bridge so the studio updates without a re-import.
 *  2. Pull: fetches Celinen verdicts/ratings/labels/captions back and writes them into
...
/** The live Celinen bridge. Lightroom runs outside the browser, so the plugin can
...
const INFO_LUA = (endpoint: string) => `--[[ Celinen — Lightroom Classic plugin ]]
return {
  LrSdkVersion = 13.0,
  LrSdkMinimumVersion = 10.0,
  LrToolkitIdentifier = 'photo.lens.os.bridge',
  LrPluginName = 'Celinen',
  LrPluginInfoUrl = '${endpoint}',
  LrLibraryMenuItems = {
    { title = 'Push selection to Celinen', file = 'CelinenPush.lua' },
    { title = 'Pull Celinen verdicts into catalog', file = 'CelinenPull.lua' },
    { title = 'Start live sync (every 5s)', file = 'CelinenWatch.lua' },
    { title = 'Stop live sync', file = 'CelinenStop.lua' },
  },
  LrMetadataProvider = 'CelinenMetadata.lua',
...
const CONFIG_LUA = (
  ...
  workspace: string,
  token: string,
) => `-- Celinen bridge settings.
-- endpoint  : the live Celinen sync URL (only change this if you self-host).
-- workspace : the studio this catalog syncs into (see Adobe tab in Celinen).
...
const METADATA_LUA = `return {
  metadataFieldsForPhotos = {
    { id = 'lensosScore', title = 'Celinen score', dataType = 'string', searchable = true, browsable = true },
    { id = 'lensosVerdict', title = 'Celinen verdict', dataType = 'string', searchable = true, browsable = true },
    { id = 'lensosPackage', title = 'Celinen package', dataType = 'string', searchable = true, browsable = true },
  },
  schemaVersion = 3,
}
`;
...
local config = require 'CelinenConfig'
...
-- Collect everything Celinen cares about for one photo.
...
-- XMP on disk keeps Lightroom and Celinen honest with each other.
...
LrTasks.startAsyncTask(function()
  ...
  if #photos == 0 or #photos > 5000 then
    LrDialogs.message('Celinen', 'Select between 1 and 5,000 photos in Lightroom first.')
    return
  end
  ...
  catalog:withWriteAccessDo('Celinen — write XMP', function()
    for _, photo in ipairs(photos) do
  ...
  if not ok then
    LrDialogs.message('Celinen', 'The local metadata write was requested, but cloud sync is not confirmed. ' .. (message or 'Check the bridge connection.'))
    return
  end
  LrDialogs.showBezel('Celinen · bridge received ' .. #frames .. ' frames; open Studio to review')
end)
`;
...
function M.plan(frames, photos)
  if type(frames) ~= 'table' or #frames == 0 or #frames > 5000 then return nil, 'No valid Celinen verdict batch. Publish again from the updated Studio.' end
  ...
  for _, frame in ipairs(frames) do
    ...
    if not relative or #relative < 2 or relative[#relative] ~= frame.file then return nil, 'Import the matching source folder in Celinen and publish again. Nothing was applied.' end
...
local LrDialogs = import 'LrDialogs'
local Bridge = require 'CelinenBridge'
local Matching = require 'CelinenMatch'

LrTasks.startAsyncTask(function()
  local state, readError = Bridge.get()
  if readError then LrDialogs.message('Celinen', readError) return end
  if state and state.kind == 'empty' then
    LrDialogs.message('Celinen', 'No verdicts are queued. Publish your selections from Celinen Studio first.')
    return
  end
  if not state or state.kind ~= 'verdicts-relative-path-v1' or not state.frames then
    LrDialogs.message('Celinen', 'No folder-matched verdicts waiting. Update the Celinen plug-in, import the matching source folder in Studio, and publish again.')
    return
  end
  local catalog = LrApplication.activeCatalog()
  local applied, failure = 0, nil
  catalog:withWriteAccessDo('Celinen — apply verdicts', function()
  ...
  if failure then LrDialogs.message('Celinen', failure) return end
  LrDialogs.showBezel('Celinen · applied ' .. applied .. ' frames')
end)
`;
...
-- Live sync: every 5 seconds push the current selection's develop state to Celinen.
LrTasks.startAsyncTask(function()
  local run = Bridge.startWatch()
  if not run then LrDialogs.showBezel('Celinen · live sync is already running') return end
  LrDialogs.showBezel('Celinen · live sync on')
  while run.active do
    local catalog = LrApplication.activeCatalog()
    local photos = catalog:getTargetPhotos()
    if photos and #photos > 5000 then
      Bridge.stopWatch(run)
      LrDialogs.message('Celinen', 'Live sync paused. Select at most 5,000 photos, then restart live sync.')
      return
    end
    if photos and #photos > 0 then
      ...
      if not ok then
        Bridge.stopWatch(run)
        LrDialogs.message('Celinen', 'Live sync paused. ' .. (message or 'The bridge did not confirm this selection.') .. ' Restart live sync after checking the connection.')
        return
      end
    end
    LrTasks.sleep(5)
...
const STOP_LUA = `local Bridge = require 'CelinenBridge'
local LrDialogs = import 'LrDialogs'
Bridge.stopWatch()
LrDialogs.showBezel('Celinen · live sync stopped; an in-flight request may still finish')
`;

const README = (endpoint: string, workspace: string) => `Celinen — Lightroom Classic plugin
==================================

Install
-------
1. Unzip. You get a folder named Celinen.lrplugin.
2. Lightroom Classic → File → Plug-in Manager → Add → choose Celinen.lrplugin → Done.
3. If Celinen is not running on this machine, open CelinenConfig.lua and change
   the endpoint. Current endpoint: ${endpoint}
   Workspace: ${workspace}  (must match the workspace shown in Celinen → Adobe)

Use
---
Library → Plug-in Extras → "Push selection to Celinen"
  Writes XMP sidecars (rating, colour label, pick flag, IPTC caption/headline/
  keywords/creator/copyright) and sends develop settings to the Celinen studio.
...
Library → Plug-in Extras → "Pull Celinen verdicts into catalog"
  Reads keeps/rejects/ratings/labels/captions back from Celinen, writes them
...
export function pluginFiles(endpoint: string, workspace: string, token: string) {
  return [
    { path: "Celinen.lrplugin/Info.lua", text: INFO_LUA(endpoint) },
    { path: "Celinen.lrplugin/CelinenConfig.lua", text: CONFIG_LUA(endpoint, workspace, token) },
    { path: "Celinen.lrplugin/CelinenBridge.lua", text: COMMON_LUA },
    { path: "Celinen.lrplugin/CelinenMatch.lua", text: MATCHING_LUA },
    { path: "Celinen.lrplugin/CelinenMetadata.lua", text: METADATA_LUA },
    { path: "Celinen.lrplugin/CelinenPush.lua", text: PUSH_LUA },
    { path: "Celinen.lrplugin/CelinenPull.lua", text: PULL_LUA },
    { path: "Celinen.lrplugin/CelinenWatch.lua", text: WATCH_LUA },
    { path: "Celinen.lrplugin/CelinenStop.lua", text: STOP_LUA },
    { path: "Celinen.lrplugin/JSON.lua", text: JSON_LUA },
...
export function downloadLightroomPlugin(credentials: { workspace: string; token: string }) {
  ...
  a.href = url;
  a.download = "Celinen.lrplugin.zip";
  a.click();
