...
### Owner activity export update · September 8, 2026

[Pixieset's Favorite Activity guide](https://help.pixieset.com/hc/en-us/articles/115002991071-How-can-I-review-my-client-s-Favorite-Activity) documents photographer-side favorite lists, notes and filename CSV export, while its [download-activity guide](https://help.pixieset.com/hc/en-us/articles/360000930212-Reviewing-Collection-Download-Activity) documents collection-level CSV for initiated gallery downloads. LensLabs now provides an owner-only filter and CSV for its existing append-only shared activity. The CSV follows the active All, Client, Selections, Feedback or Downloads filter, excludes private upload reservations, quotes every field, neutralizes spreadsheet-formula prefixes, and preserves the explicit statement that a browser handoff did not verify the final save location.

This is an activity audit, not a filename-only Adobe handoff or proof that a client saved a file. It was verified with synthetic loopback history at a 390-pixel viewport plus focused domain tests. No Pixieset or Aftershoot account, production invitation, client email, customer image or live storage was used.
