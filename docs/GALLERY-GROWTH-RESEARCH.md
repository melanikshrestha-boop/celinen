...
Validation for this increment: 717 automated tests pass; production build, TypeScript, targeted lint, and isolated delivery-outbox checks pass. New regressions cover branding permissions, private-link handling, safe signup destination, escaped identity, credit-off behavior, delayed clipboard receipts, and observed invitation rotation. The local proofing route responds successfully. Browser testing of this new increment was requested but not performed without a response; no live-provider or real-client verification is claimed.
