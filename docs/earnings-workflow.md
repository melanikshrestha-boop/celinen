...
- Collected is period cash receipts less individually dated successful refunds. Net subtracts recorded expenses, before tax and unrecorded fees. Never sum different currencies.
...
Use one shared sans-serif, neutral dark/light surfaces, restrained selected states and a compact payment pulse. Manual data entry is in a drawer, not a spreadsheet footer. A row opens details and the linked shoot. Hover-only pin/archive controls apply per exact chat/shoot row, with keyboard-focus and touch access, persistent organization, archive recovery and Undo.

## Recorded-data charts

Earnings uses pitch black (`#000000`) in dark mode and neutral white (`#fafafa`) in light mode, independent of saved canvas/background colors. No appearance preferences or financial records are rewritten.

Insights shows a cash-flow bar/line graph plus Income Mix and Expense Mix donut pies. This Month uses calendar days; This Year and All Dates use calendar months. Charts follow the period, currency and shoot filters; ledger tabs and text search only filter the ledger. The graph shows collections after refunds, expenses and net. The pies show positive recorded category amounts; dated refunds and negative historical corrections are disclosed separately and reconcile to the pulse. Empty, incomplete and loading states never render invented chart data.
